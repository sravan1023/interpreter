...
typedef int32_t     pid32;      /* Process ID */
typedef int32_t     status;     /* Status return type */
#define OK          1           /* System call succeeded */
#define SYSERR      -1          /* System call failed */
#endif


#define SHELL_MAX_LINE      256         /* Maximum command line length */
#define SHELL_MAX_ARGS      32          /* Maximum command arguments */
#define SHELL_MAX_CMD       64          /* Maximum command name length */
#define SHELL_MAX_PATH      256         /* Maximum path length */
#define SHELL_HISTORY_SIZE  50          /* Command history entries */
#define SHELL_MAX_ALIAS     32          /* Maximum number of aliases */
...
#define SHELL_OK            0           /* Command succeeded */
#define SHELL_ERROR         1           /* Command failed */
#define SHELL_EXIT          -1          /* Exit shell */
#define SHELL_NOT_FOUND     127         /* Command not found */


typedef int (*shell_cmd_func)(int argc, char **argv);

typedef struct shell_command {
    char            name[SHELL_MAX_CMD];    /* Command name */
    char            description[128];       /* Command description */
    shell_cmd_func  func;                   /* Command function */
    bool            builtin;                /* Is built-in command */
} shell_command_t;


typedef struct shell_alias {
    char    name[SHELL_MAX_CMD];            /* Alias name */
    char    value[SHELL_MAX_LINE];          /* Alias expansion */
...
typedef struct shell_state {
    ...
    int32_t         last_exit;              /* Last command exit status */
    pid32           pid;                    /* Shell process ID */
...
#define SCRIPT_MAX_VARS     128         /* Maximum variables */
#define SCRIPT_MAX_FUNCS    64          /* Maximum functions */
#define SCRIPT_MAX_STACK    256         /* Execution stack size */
#define SCRIPT_VAR_NAME_LEN 64          /* Variable name length */
#define SCRIPT_VAR_VAL_LEN  256         /* Variable value length */
#define SCRIPT_MAX_LINE     512         /* Maximum line length */
#define SCRIPT_MAX_LABELS   64          /* Maximum labels */

typedef enum {
    VAR_TYPE_INT,                       /* Integer */
    VAR_TYPE_STRING,                    /* String */
    VAR_TYPE_FLOAT,                     /* Floating point */
    VAR_TYPE_ARRAY,                     /* Array */
    VAR_TYPE_UNDEFINED                  /* Not defined */
} var_type_t;
...
/* Built-in commands */
extern int      shell_register_command(const char *name, const char *desc,
                                       shell_cmd_func func);
...
/* Variables */
extern int      script_set_var(script_context_t *ctx, const char *name,
                               var_type_t type, void *value);
extern int      script_get_var(script_context_t *ctx, const char *name,
                               var_type_t *type, void *value);
...
/* Functions */
extern int      script_define_func(script_context_t *ctx, const char *name,
                                   const char *body, int num_params);
extern int      script_call_func(script_context_t *ctx, const char *name,
                                 int argc, char **argv);
